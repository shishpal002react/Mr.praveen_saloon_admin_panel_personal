...
const OfferSection = () => {
  ...
  const getdata = async () => {
    try {
      const response = await axios.get(
        `${Baseurl}api/v1/admin/Banner/all/offerBanner`,
        {
          headers: {
  ...
  return (
    <>
      <AddOffer show={show} onHide={() => setShow(false)} getdata={getdata} />
      ...
      {/* hjju */}
        <div className="pc2">
          <p style={{ fontSize: "18px", fontWeight: "Bold" }}>Offer Banner</p>

...
const HeroBanner = () => {
  ...
  const getdata = async () => {
    try {
      const response = await axios.get(
        `${Baseurl}api/v1/admin/Banner/all/heroBanner`,
        {
          headers: {
      ...
      setBanner(response.data.data.reverse());
    } catch {}
  };
  ...
  return (
    <>
      <AddBanner show={show} onHide={() => setShow(false)} getdata={getdata} />
      <Navbar />
      <div className="pc1">
        {/* juhi */}
        <div className="pc2">
          <p style={{ fontSize: "18px", fontWeight: "Bold" }}>Hero Banner</p>
